...
const stripePromise = loadStripe('pk_test_51RL2aDIM0tDpIeUTCaehMWLODxmWVPrNXUAOyjJfdBKzjQDmYqJxmAm6I1TGuoS9bKfHD4GzGKFpPmuRKk6vvxRn00bASthcUy'); 
...
const Checkout = () => {
  ...
  const handleStripeOrder = async () => {
    ...
    try {
      const res = await axios.post(`${backendUrl}/api/order/stripe`, {
        items: cartItems,
        address,
        amount: getTotalPrice(),
      ...
      if (res.data.success) {
        ...
        alert("Stripe order failed: " + res.data.message);
      }
    } catch (err) {
